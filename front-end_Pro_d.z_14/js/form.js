class Student {
    ...
    summary() {
        ...
        const averageAttendance = this.attendance.filter(Boolean).length / this.attendance.filter(item => item !== null).length;
...
const student1 = new Student('Сергій', 'Омельченко', 1993, [100, 100, 100, 100, 100]);
    for (let i = 0; i < 25; i++) student1.present();
    for (let i = 0; i < 0; i++) student1.absent();
...
const student2 = new Student('Ярослав', 'Демченко', 1994, [81, 82, 83, 84, 85]);
    ...
    for (let i = 0; i < 0; i++) student2.absent();
